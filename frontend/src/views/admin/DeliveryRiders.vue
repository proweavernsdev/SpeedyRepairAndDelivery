  ...
  ">
    ...
    <template #Modal-Body>
      <div v-show="StatusView == true" class="px-6">
        <div
          class="grid grid-cols-2 py-3 gap-y-1 [&>label]:col-start-1 [&>h2]:col-start-2 [&>h2]:text-[#AA0927] text-lg [&>*]:p-5"
          v-for="items in data" :key="items.id">
          <label for="">ID:</label>
          <h2>{{ items.driverID }}</h2>
          <label for="">Email:</label>
          <h2>{{ items.comp_address }}</h2>
          ...
          <SRTable>
            ...
            <template #Table-Body>
              <tr v-for="items in data" :key="items.id" class="[&>*]:p-5">
                <td class="text-lg">{{ items.driverID }}</td>
                <td>{{ items.driver_lastName }}, {{ items.driver_firstName }}</td>
                <td>{{ items.driver_address }}</td>
                ...
                <td>
                  <div
                    ...
                    <button class="hover:bg-rose-600" @click="update(items.driverID, 'approved')">
                      <i v-html="icons.ThumbsUp"></i>
                    </button>
                    <button class="hover:bg-rose-600" @click="update(items.driverID, 'denied')">
                      <i v-html="icons.ThumbsDown"></i>
                    </button>
                    <button class="hover:bg-rose-600" @click="update(items.driverID, 'blocked')">
                      <i v-html="icons.Forbidden"></i>
                    </button>
                    <button class="hover:bg-rose-600" @click="update(items.driverID, 'on_hold')">
...
function update(driverID, status) {
