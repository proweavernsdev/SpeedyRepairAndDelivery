<template>
  ...
  <SRContents v-if="userStatus == null">
    <template #Content-Header>
      <h2>Dashboard</h2>
    </template>
    <template #Content-Body>
...
onMounted(() => {
  ...
  promise
    .then((data) => {
      if (data.data.data.UserAccess !== undefined) {
        const param = route.params.id;
        const status = data.data.data.UserAccess;
